        ...
        it('should remove logs older than specified days', async () => {
            ...
            await new Promise((resolve, reject) => {
                database.db.serialize(() => {
                    ...
                    database.db.run(
                        `INSERT INTO logs (guild_id, action_type, moderator_id, target_user_id, details, timestamp)
                         VALUES (?, 'old_action', 'mod_2', 'user_2', '{}', datetime('now', '-35 days'))`,
                        [guildId]
                    );
                    
                    // Another old log (should be deleted)
                    database.db.run(
                        `INSERT INTO logs (guild_id, action_type, moderator_id, target_user_id, details, timestamp)
                         VALUES (?, 'very_old_action', 'mod_3', 'user_3', '{}', datetime('now', '-60 days'))`,
                        [guildId],
                        (err) => {
                            if (err) reject(err);
                            else resolve();
            ...
            const deletedCount = await database.cleanupOldLogs(30);
            ...
            const remainingLog = await new Promise((resolve, reject) => {
                database.db.get(
                    'SELECT action_type FROM logs WHERE guild_id = ?',
                    [guildId],
                    (err, row) => {
                        if (err) reject(err);
                        else resolve(row);
            ...
            expect(remainingLog.action_type).toBe('recent_action');
        });

        it('should return 0 when no logs need cleanup', async () => {
            // Insert a recent log
            await database.logAction('test_guild', 'recent_action', 'mod_1', 'user_1', {});

            // Try to clean up logs older than 1 day
            const deletedCount = await database.cleanupOldLogs(1);
