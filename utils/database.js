...
class Database {
    ...
    async incrementMessageCount(guildId, userId, date) {
        return new Promise((resolve, reject) => {
            ...
            db.run(query, [guildId, userId, date], function(err) {
                ...
                const getQuery = 'SELECT message_count FROM daily_messages WHERE guild_id = ? AND user_id = ? AND date = ?';
    ...
    async getMessageCount(guildId, userId, date) {
        return new Promise((resolve, reject) => {
            ...
            const query = 'SELECT message_count FROM daily_messages WHERE guild_id = ? AND user_id = ? AND date = ?';
    ...
    async logAction(guildId, actionType, moderatorId, targetUserId, details) {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(query, [guildId, actionType, moderatorId, targetUserId, detailsJson], function(err) {
                if (err) {
    ...
    async cleanupOldLogs(daysToKeep = 30) {
        return new Promise((resolve, reject) => {
            ...
            const query = 'DELETE FROM logs WHERE timestamp < datetime("now", "-" || ? || " days")';
