...
function getHelpEmbed(){
	return new EmbedBuilder()
		...
		.addFields(
			{ name : '```/messagequota {quota} ```', value: 'Set message quota from 0 to 1000 messages per day. 0 means no quota. Can only be used by a user with Admin permission.'},
			...
			{ name : '```/unwatchlist {required: user_name} {optional: reason}```', value: 'Remove "watchlist" role from the user. Also clears their daily message amount. Requires username of targeted user. Reason is optional. Can be used by users with "mute" permission.'},
			{ name : '```/watchlist {required: user_name} {optional: reason}```', value: 'Add "watchlist" role to the user, so their message amount is tracked against the daily quota. Requires username of targeted user. Reason is optional. Can be used by users with "mute" permission.'},
